...
function generateHubPages({root,outputPath,data,hubIndex}) {
 ...
 for(const type of ['province','region','subject','target'])for(const hub of hubIndex[type]){
  ...
  const subjectNames=[...new Set(cluster.map(p=>p.subject))].join('·');
  const copies={
   province:[`${label} 과외 | 지역별 ${subjectNames} 수업 찾기`,`${label} 과외 지역 찾기`,`${label}에 속한 지역에서 초등학생·중학생·고등학생 ${subjectNames} 수업을 찾아보세요. 지역별 페이지에서 학년·과목별 1:1 과외의 학습 방향을 비교할 수 있습니다.`],
   region:[`${label} 1:1 ${subjectNames} 과외 | 초·중·고 수업`,`${label} 1:1 ${subjectNames} 과외`,`${hub.province} ${label}의 초등학생·중학생·고등학생 ${subjectNames} 수업을 학교급과 학습 목표에 맞춰 찾아보세요. 학년·과목별 수업의 학습 방향을 비교할 수 있습니다.`],
   subject:[`${label}과외 | 초등·중등·고등 학습 방향과 지역별 수업`,`${label}과외, 학교급에 맞는 학습 방향`,`${label}과외의 초등·중등·고등 학습 차이를 확인하고 지역별 1:1 수업을 탐색하세요. ${label==='국어'?'어휘와 문해력부터 문학·독서, 학교 내신과 수능까지 안내합니다.':label==='영어'?'기초 어휘와 읽기부터 문법·구문독해, 내신과 수능까지 안내합니다.':'연산과 기초개념부터 유형별 문제풀이, 내신과 수능의 풀이 전략까지 안내합니다.'}`],
   target:[`${label} 과외 | ${subjectNames} 학습 방향과 지역 안내`,`${label} ${subjectNames} 과외`,`${label}에게 필요한 ${subjectNames}의 학습 방향을 비교해 보세요. ${gradeCopy[label]||''} 지역별 수업과 상담 안내도 함께 확인할 수 있습니다.`],
  };
  const [title,h1,description]=copies[type];let content='';
  if(type==='province')content=`<div class="hub-page-grid">${hubIndex.region.filter(h=>h.province===hub.value).map(h=>`<article class="hub-page-card"><h3>${a(h.url,h.region+' 과외')}</h3><p>초등·중등·고등 ${subjectNames} 수업 둘러보기</p></article>`).join('')}</div>`;
  else if(type==='region'){
   const order=['elementary','middle','high'].flatMap(level=>TYPES.filter(t=>t.startsWith(level+'_')));
  ...
  const hubFaqs=[{question:`${label} 수업은 어떻게 찾나요?`,answer:type==='province'?'지역을 선택한 뒤 해당 지역의 학교급과 과목별 수업을 비교해 보세요.':type==='region'?`초등·중등·고등과 ${subjectNames}의 수업 중 학생에게 필요한 학습 방향을 확인해 보세요.`:'학교급과 과목의 학습 방향을 확인한 뒤 시도별 지역 목록을 펼쳐 수업 페이지로 이동하세요.'},faqs[4],faqs[6]];
